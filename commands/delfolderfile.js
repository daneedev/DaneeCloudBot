...
new Command({
	name: 'delfolderfile',
	description: 'Delete a file in folder',
	...
	run: async (ctx) => {
        ...
        if (ctx.member.permissions.has(process.env.admin_perm)) {
            const delfile = await cloud.deleteFileFolder(username, folder, file)
            if (delfile.code == 200) {
            const embed = new Discord.EmbedBuilder()
                .setTitle(`File ${file} in /${folder} has been deleted`)
                .setColor("#5D3FD3")
